...
fetch('data.json')
  ...
  .then(data => {
    ...
    const energies = [...new Set(data.map(d => d.energy))].sort((a, b) => a - b);
    const projList = document.getElementById('projectiles');
    const tarList = document.getElementById('targets');
    const energyList = document.getElementById('energies');
    ...
    energies.forEach(e => {
      const opt = document.createElement('option');
      opt.value = e;
      energyList.appendChild(opt);
    });
  })
  .catch(err => console.error("Failed to load data.json:", err));

// 元素名到符号的映射（支持大小写）
const elementNamesToSymbol = {
  // 英文名 → 符号
  'Hydrogen': 'H', 'Deuterium': 'd', 'Tritium': 't',
  'Helium': 'He', 'Alpha': 'alpha',
  ...
  'Iron': 'Fe', 'Copper': 'Cu', 'Uranium': 'U', 'Lead': 'Pb',
  // 添加更多...
};

// 反向映射：符号 → 常见名称（用于提示）
const symbolToName = Object.fromEntries(
  Object.entries(elementNamesToSymbol).map(([name, sym]) => [sym, name])
);  
...
document.getElementById('queryForm').onsubmit = function(e) {
  e.preventDefault();
  const form = e.target;
  let projectile = form.projectile.value.trim();
  let target = form.target.value.trim();
  ...
  if (elementNamesToSymbol[projectile]) {
    projectile = elementNamesToSymbol[projectile];
  }
  if (elementNamesToSymbol[target]) {
    target = elementNamesToSymbol[target];
  }
  if (results.length === 0) {
...
// 按靶核查询所有反应
document.getElementById('queryByTargetBtn').onclick = function() {
  const targetInput = document.getElementById('queryByTarget');
  ...
  if (!target) return alert("请输入靶核");

  const results = fullData.filter(d => d.target === target);

  if (results.length === 0) {
    alert(`未找到靶核为 ${target} 的数据`);
    return;
  }

  // 按 projectile + energy 分组
  const grouped = {};
  results.forEach(r => {
    const key = `${r.projectile}@${r.energy}MeV`;
  ...
  Object.entries(grouped).forEach(([key, data]) => {
    const [projectile, energyStr] = key.split('@');
    const energy = parseFloat(energyStr);

    const trace = {
      x: data.map(r => r.A),
      y: data.map(r => r.yield),
      mode: 'lines+markers',
      type: 'scatter',
      name: `${projectile} + ${target} @ ${energy} MeV`,
      text: data.map(r => `${r.product} (Z=${r.Z})`),
